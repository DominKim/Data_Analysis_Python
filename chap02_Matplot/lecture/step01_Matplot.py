...
plt.plot(data, data2, "bo") # (x, y)
plt.show() 

plt.plot(data, data2, "go")
plt.show()


# 2. 산점도 그리기
# 단색 산점도
plt.scatter(data, data2, c = "hotpink", marker = "o")
plt.show()
...
# 3. subplot 이용 차트 그리기 : 격자 형식 차트
fig = plt.figure(figsize = (5, 3)) # 차트 크기 (가로,세로)
...
plt.show()
# ipython console > graphics > Backend (automatic으로 변경)!!
# 음수값 표현
...
chart2.bar(idx, data, color = "darkblue") # bar(x, height, width)

# x축 눈금 레이블
x_label = ["서울", "대전", "부산", "광주", "인천"]
plt.xticks(idx, x_label)
plt.xlabel("판매 지역")
plt.ylabel("지역별 매출 현황")
plt.title("202년 1분기 전국 판매현황")
